...
import { Database, Code, FileJson, RefreshCw, Trash2, Terminal, CheckCircle, Wand2, Zap, PlayCircle, SearchCheck, FileCode, Settings, Eye, EyeOff, FileText, ShieldOff, FileSearch, ListOrdered, Rocket, Sparkles, Filter, Archive, Grid3x3, Github, FolderOpen } from 'lucide-react';
...
import { GitHubSyncWriter } from './GitHubSyncWriter';
import { FileManager } from './FileManager';

type ToolView = 'none' | 'connection' | 'setup' | 'schema' | 'sync' | 'githubsync' | 'githubwriter' | 'filemanager' | 'reset' | 'sql' | 'verify' | 'wizard' | 'direct' | 'oneclick' | 'autorun' | 'verifier' | 'completescript' | 'rlskiller' | 'manualverifier' | 'stepbystep' | 'ultimatesql' | 'scriptrunner';

type ToolCategory = 'all' | 'sql' | 'scripts' | 'setup' | 'verification' | 'maintenance' | 'sync' | 'files' | 'archived';
...
export function DevToolsIntegration() {
  ...
  const tools: Tool[] = [
    ...
    // 🔄 SINCRONIZACIÓN
    {
      ...
      description: 'Sincronizar documentación y assets desde el repositorio remoto',
      icon: Github,
    ...
    {
      id: 'githubwriter',
      title: 'GitHub Sync Writer',
      description: 'Escribir cambios en el repositorio remoto',
      icon: Github,
      color: 'from-gray-600 to-slate-600',
      bgColor: 'bg-slate-900/20',
      textColor: 'text-white',
    ...
    // 📁 FILE MANAGEMENT
    {
      id: 'filemanager',
      title: 'File Manager',
      description: 'Gestionar archivos sin intervención del agente (Upload, GitHub, URL, Storage)',
      icon: FolderOpen,
  ...
  const categories = [
    ...
    { id: 'files' as ToolCategory, name: 'Archivos', icon: FolderOpen, count: tools.filter(t => t.category === 'files' && t.status !== 'archived').length },
    ...
    <div className="space-y-6">
      ...
      {activeView !== 'none' && (
        <div className="bg-slate-800 rounded-2xl border-2 border-slate-700 p-6">
          ...
          {activeView === 'githubsync' && <GitHubSync />}
          {activeView === 'githubwriter' && <GitHubSyncWriter />}
