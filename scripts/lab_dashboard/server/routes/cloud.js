...
const os = require('os');
const MountManager = require('../services/MountManager'); // V11 Import
...
// V10: Helper to list from Native Mount
const listFromMount = async (prefix) => {
    try {
        const mountPath = '/mnt/cloud';
        const targetDir = path.join(mountPath, prefix);

        // Security check
        if (!targetDir.startsWith(mountPath)) throw new Error('Invalid path access');
        ...
        for (const item of items) {
            ...
            const fullKey = path.join(prefix, item.name).replace(/\\/g, '/');

            if (item.isDirectory()) {
                folders.push({
                    key: fullKey + '/', // Ensure trailing slash
                    name: item.name,
                    type: 'folder',
                    folderCount: 0, // Native doesn't pre-calc deep stats instantly
                    fileCount: 0,
                    isComplete: true
                });
            } else {
        ...
        return { folders, files, source: 'native-mount' };

    } catch (err) {
        console.warn(`[NativeMount] Failed to list ${prefix}:`, err.message);
        return null;
    }
};
...
module.exports = (dependencies) => {
    ...
    // V11: Settings Endpoint for Native Mount
    let USE_NATIVE_MOUNT = true; // Default

    router.get('/settings/mount', (req, res) => {
        res.json({
            useNativeMount: USE_NATIVE_MOUNT && MountManager.isReady(),
            isMounted: MountManager.isReady(),
            mountStatus: MountManager.getStatus()
        });
    });

    router.post('/settings/mount', (req, res) => {
        const { enabled } = req.body;
        USE_NATIVE_MOUNT = !!enabled;
        broadcastLog('system', `Mount Preference changed to: ${USE_NATIVE_MOUNT ? 'NATIVE' : 'LEGACY S3'}`, 'info');
        res.json({ success: true, useNativeMount: USE_NATIVE_MOUNT });
    });

    // --- LIST TREE (Directory Listing) ---
    router.get('/list/tree', async (req, res) => {
        const prefix = req.query.prefix || '';

        try {
            // V10/V11: Try Native Mount first (if enabled and ready)
            if (USE_NATIVE_MOUNT && MountManager.isReady()) {
                console.log(`[CloudList] Attempting Native Mount read for: ${prefix}`);
                const nativeData = await listFromMount(prefix);

                if (nativeData) {
                    return res.json({
                        prefix,
                        folders: nativeData.folders,
                        files: nativeData.files,
                        cacheStatus: { isReady: true, instant: true, source: 'native-mount' }
                    });
                } else {
                    console.log('[CloudList] Native read failed/empty, falling back to cache/S3');
                }
            }
    ...
    router.post('/upload/batch', dependencies.multer().array('files'), async (req, res) => {
        ...
        try {
            ...
            const uploadPromises = req.files.map((file, index) => {
                ...
                // V11: Future - Use Native Write here if enabled
