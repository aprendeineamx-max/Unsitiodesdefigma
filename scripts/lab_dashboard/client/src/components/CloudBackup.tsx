...
export const CloudBackup: React.FC<CloudBackupProps> = ({ versionId, versions }) => {
    ...
    const [storageInfo, setStorageInfo] = useState({ used: 0, total: 1000000 }); // 1TB Limit as requested
        ...
        <div className="flex h-full bg-white dark:bg-slate-900 overflow-hidden relative">
            ...
            <div className="w-64 border-r border-gray-200 dark:border-slate-800 flex flex-col bg-slate-50 dark:bg-slate-900/50">
                ...
                <nav className="flex-1 px-2 space-y-1">
                    <button
                        onClick={() => { setActiveTab('drive'); setFilter('all'); }}
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'drive' && filter === 'all' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                    >
                        <HardDrive className="w-4 h-4" /> All Files
                    </button>
                    <button
                        onClick={() => { setActiveTab('drive'); setFilter('uploads'); }}
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'drive' && filter === 'uploads' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                    >
                        <Upload className="w-4 h-4" /> Uploads
                    </button>
                    <button
                        onClick={() => { setActiveTab('drive'); setFilter('backups'); }}
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'drive' && filter === 'backups' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                    >
                    ...
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'sync' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                    >
